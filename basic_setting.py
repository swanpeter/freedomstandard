...
class BasicSetting:
    @staticmethod
    def _sanitize_max_age_seconds(value: object, default: int) -> int:
        try:
            parsed = int(value)
    ...
    def __init__(
        ...
        login_cookie_max_age_seconds: int = 60 * 60 * 24 * 30,
        ...
        self.login_cookie_max_age_seconds = self._sanitize_max_age_seconds(
            login_cookie_max_age_seconds,
            60 * 60 * 24 * 30,
        )
        self.session_cookie_max_age_seconds = self._sanitize_max_age_seconds(
    ...
    def _cookie_expiry(self, max_age_seconds: int) -> datetime.datetime:
        return datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(seconds=max_age_seconds)
        ...
        try:
            if value:
                controller.set(
                    self.cookie_key,
                    "1",
                    expires=self._cookie_expiry(self.login_cookie_max_age_seconds),
                    max_age=float(self.login_cookie_max_age_seconds),
                    same_site="lax",
                )
                time.sleep(0.6)
            else:
                controller.remove(self.cookie_key, same_site="lax")
        except Exception:
            return
    ...
    def get_browser_session_id(self, create: bool = True) -> Optional[str]:
        ...
        try:
            controller.set(
                ...
                new_id,
                expires=self._cookie_expiry(self.session_cookie_max_age_seconds),
                max_age=float(self.session_cookie_max_age_seconds),
                same_site="lax",
            )
            time.sleep(0.6)
        except Exception:
...
def cookie_controller_available() -> bool:
    return _default_container.cookie_controller_available()
